...
P_HEADS, P_TAILS, P_EDGE = 0.495, 0.495, 0.01
K_MAIN, K_EDGE = 1.75, 8.0
...
async def db_init():
    async with db_conn() as db:
        await db.execute("CREATE TABLE IF NOT EXISTS users(uid INTEGER PRIMARY KEY, balance INTEGER DEFAULT 0)")
        await db.execute("""CREATE TABLE IF NOT EXISTS bets(
            id INTEGER PRIMARY KEY AUTOINCREMENT, uid INTEGER, side TEXT, stake INTEGER,
            outcome TEXT, prize INTEGER, ts DATETIME DEFAULT CURRENT_TIMESTAMP)""")
        await db.commit()
...
async def add_balance(uid, delta):
    async with db_conn() as db:
        await db.execute("""INSERT INTO users(uid,balance) VALUES(?,?)
            ON CONFLICT(uid) DO UPDATE SET balance=balance+excluded.balance""", (uid, delta))
        await db.commit()
...
async def choose_side(c: CallbackQuery):
    ...
    await c.answer()
    await c.message.answer(f"Выбрано: <b>{names.get(side, side)}</b>. Пришли сумму ставки.")
...
async def cb_topup(c: CallbackQuery):
    ...
    await c.answer()
    await c.message.answer(f"✅ Пополнение: +{amount} XTR\nБаланс: <b>{bal}</b> XTR", reply_markup=main_menu_kb(bal))

@dp.callback_query(F.data == "withdraw")
async def cb_withdraw(c: CallbackQuery):
    bal = await get_balance(c.from_user.id)
    await c.answer()
    await c.message.answer("🔄 Вывод тестовый (макет).", reply_markup=main_menu_kb(bal))
